...
describe('Integration tests', function () {
    ...
    const sampleProjectsRoots = fs.readdirSync(sampleProjectsPath)
        .map(dirName => path.join(sampleProjectsPath, dirName));
    ...
    for (const sampleProjectPath of sampleProjectsRoots) {
        ...
        it(`Test sample project ${baseName}`, function () {
            const projectPath = baseName === 'relative-path-outside-project' 
                ? path.join(sampleProjectPath, 'project') 
                : sampleProjectPath;
